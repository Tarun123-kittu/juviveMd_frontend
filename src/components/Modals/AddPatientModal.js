    ...
    useEffect(() => {
        if (onboarding_process?.isSuccess) {
            if (step === 2) {
                ...
                setStepThreeFullData()
                setSelected_health_issue("")
                dispatch(get_patients_list({ page: 1, tab: "healthIssue" }))
            ...
            if (step === 4) {
                ...
                setStepThreeFullData()
                setSelected_health_issue("")
                dispatch(get_patients_list({ page: 1, tab: "paymentPending" }))
                dispatch(clear_patient_onboarding_state())
                handleClose()
                setWorkout_frequency()
                setStep_four_additional_information("")
        ...
        <div>
            <Modal show={showPateintModal} onHide={handleClose} className="cmn_modal addPatient" centered size="md">
                ...
                <Modal.Body className="p-0">
                    ...
                    {is_health_issue && <ConfirmForm setStep={setStep} step_form_open={step_form_open} setIs_health_issue={setIs_health_issue} setStep_form_open={setStep_form_open} setshowPateintModal={setshowPateintModal} handleConfirm={handleSubmit} onboarding_process={onboarding_process}/>}
                </Modal.Body>
            </Modal>
        </div>
    );
