...
const DeleteModal = ({ showDeleteModal, setshowDeleteModal, handleDelete, loading, text, setShowDeleteModal }) => {
    const handleClose = () => {
        ...
        <div>
            <Modal show={showDeleteModal} onHide={handleClose} className="cmn_modal deleteModal" centered size="md">
                ...
                <Modal.Body className="p-0">
                    ...
                    {!text && <h5 className="mb-3 deletmodal_heading mt-4 text-center">Are You Sure You Want to <br /> Delete This User?</h5>}
                    {text && <h5 className="mb-3 deletmodal_heading mt-4 text-center">Are You Sure You Want to Logout</h5>}
                    {!text && <p className="text-center">This account will be deleted permanently from your dashboard.</p>}
                    {!loading ? <button className="cmn_btn w-100" onClick={() => handleDelete()}>{text ? "Logout" : "Delete"}</button>
                        :
                        <button className="cmn_btn w-100">  <Spinner animation="border" role="status">
                            <span className="visually-hidden">Loading...</span>
                        </Spinner></button>}
                    {!text && < button className="cmn_btn fade_color w-100 mt-3" onClick={() => handleClose()}>Cancel</button>}
                    {text && < button className="cmn_btn fade_color w-100 mt-3" onClick={() => setShowDeleteModal(false)}>Cancel</button>}
            </Modal.Body>
        </Modal>
        </div >
    );
