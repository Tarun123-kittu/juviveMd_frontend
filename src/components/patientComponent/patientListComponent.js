            ...
            <div className={showButtons ? 'wrapper' : ''}>
                <div className={showButtons ? 'inner_wrapper' : ''}>
                    <div className="cmn_head d-flex align-items-center mb-3 position-relative gap-3">
                        <h2 className='flex-grow-1'>Patient List sadsa</h2>
                    </div>
                    <div className="cmn_head d-flex align-items-center  position-relative gap-3 mb-3">
                        ...
                        {PatientPermissions?.canCreate && showButtons && <button onClick={() => setshowPateintModal(true)} className='cmn_btn filter_btn'>+ Add Patient</button>} {showButtons && <button onClick={handelShowFilter} className="cmn_btn filter_btn px-4">Filter</button>}
                        {showFilter &&
                                    ...
                                    <tr>
                                        ...
                                        <td className='text-center'>
                                            <div className="d-flex gap-3 w-100 align-items-center">
                                                ...
                                                {PatientPlan?.canRead && tab !== "healthIssue" && showButtons && <button   disabled={patient?.status === 0 && patient?.payment === false} onClick={() => {navigate("/patientData", { state: { patientId: patient?.id, val: tab } })}} className={`${(patient?.status === 0 || patient?.payment === false) && 'disable_opacity'} cmn_btn px-2 py-1`} style={{ height: "36px" }}> <img width="18" src={ArrowRight} /> </button>}
                                            </div>
                                        </td>
                                    </tr>
                                )
