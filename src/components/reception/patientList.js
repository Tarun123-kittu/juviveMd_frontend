            ...
            <div className='wrapper'>
                <div className='inner_wrapper'>
                    ...
                    <DataTable columns={columns}>
                        {patient_data?.isLoading ? <tr><td colSpan={9}><Loader /></td></tr> : patient_data?.data?.data?.items?.length === 0 ? <td><tr colSpan={9}><img className='text-center' src={No_data_found} alt="" /> </tr></td> : Array.isArray(patient_data?.data?.data?.items) && patient_data?.data?.data?.items?.map((patient, i) => {
                            return (
                                <tr>
                                    <td className="ps-3">
                ...
                <EditpatientModal showPateintModal={showEditPateintModal} setshowPateintModal={setshowEditPateintModal} tab={tab} common_data={common_data} patientId={patientId}/>
            </div>
        </div>
    )
}
