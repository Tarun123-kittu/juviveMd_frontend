...
import {
  ...
  clear_selected_patient_state,
} from "../../redux/slices/patientSlice/getSelectedPatientSlice";
...
import {
  get_patient_plan,
  clear_patient_plan_state,
} from "../../redux/slices/patientPlan/getPAtientPlan";
...
import {
  isPatientPlanEditable,
  clear_is_patient_plan_editable,
} from "../../redux/slices/patientPlan/editPatientPlan";
...
const PatientData = () => {
  ...
  const [isPatientPlanEditableDate, setIsPatientPlanEditableDate] =
    useState("");
  ...
  const getDateForDay = (dayName) => {
    ...
    targetDate.setDate(
      startOfWeek.getDate() + (targetDayIndex === 0 ? 6 : targetDayIndex - 1)
    );
  ...
  const patient_details = useSelector(
    (store) => store.SELECTED_PATIENT_DETAILS
  );
  const common_data = useSelector((store) => store.COMMON_DATA);
  const isPatientPlanEditableResponse = useSelector(
    (store) => store.IS_PATIENT_PLAN_EDITABLE
  );
  const [patientPlanPermissions] = getRoutePermissions(
    permission_constants.PATIENTPLAN
  );
  const [patientReportsPermissions] = getRoutePermissions(
    permission_constants.PATIENTPLAN
  );
  ...
  useEffect(() => {
    if (isPatientPlanEditableResponse?.isSuccess && isPatientPlanEditableDate) {
      const planStartDate =
        isPatientPlanEditableResponse?.data?.data?.planValidFrom;
      const planEndDate =
        isPatientPlanEditableResponse?.data?.data?.planValidTo;
      setPlanStartAt(planStartDate);
      setPlanEndAt(planEndDate);
      setExercisePlanId(isPatientPlanEditableResponse?.data?.data?.id);

      if (
        isPatientPlanEditableDate > planStartDate &&
        isPatientPlanEditableDate < planEndDate
      ) {
  ...
  useEffect(() => {
    if (patient_details?.isSuccess) {
      ...
      const bmi = calculateBMI(
        patient_details?.data?.data?.height?.value,
        patient_details?.data?.data?.weight?.value,
        patient_details?.data?.data?.height?.unit,
        patient_details?.data?.data?.weight?.unit
      );
      setCurrentBmi(bmi.bmi);
      const expectbmi = calculateBMI(
        patient_details?.data?.data?.height?.value,
        patient_details?.data?.data?.optimal_weight?.value,
        patient_details?.data?.data?.height?.unit,
        patient_details?.data?.data?.optimal_weight?.unit
      );
  ...
  return (
    <div className="wrapper">
      <div className="inner_wrapper">
                  ...
                  <div className="d-inline-grid">
                    ...
                    <div className="info_flex_grid align-items-center mt-2">
                      ...
                      <div>
                        ...
                        <p className="m-0">
                          {patient_data?.countryCode ? "+" : ""}
                          {patient_data?.countryCode} {patient_data?.phone}
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  <ul className="user_select_weeks">
                    {daysOfWeek.map((day) => (
                      <li
                        key={day}
                        className={
                          patient_data?.exercise_perweek?.includes(
                            Object.keys(fullToAbbr).find(
                              (key) => fullToAbbr[key] === day
                            )
                          )
                            ? "selected_day"
                            : ""
              ...
              <div className="user-data">
                ...
                <div className="d-flex gap-3 justify-content-between">
                  <ul className="">
                    ...
                    {patient_data?.fat_percentage?.value > 0 && (
                      <li>
                    ...
                    <li>
                      ...
                      <span>
                        {patient_data?.gender
                          ? patient_data?.gender?.charAt(0).toUpperCase() +
                            patient_data?.gender.slice(1).toLowerCase()
                          : ""}
                      </span>
                    </li>
                  </ul>
                  <ul>
                    <li>
                      ...
                      <span>
                        {patient_data?.optimal_weight?.value}{" "}
                        {patient_data?.optimal_weight?.unit}
                      </span>
                    </li>
                    <li>
              ...
              <div className="bmi_data ">
                <ul className="align-items-center d-flex gap-2 h-100">
                  <li className="bmi_card flex-grow-1">
                    ...
                    <span>Height</span>
                    <p className="mb-0">
                      {patient_data?.height?.value} {patient_data?.height?.unit}
                    </p>
                  </li>
                  <li className="bmi_card flex-grow-1">
                    <div className="link_image d-flex justify-content-center align-items-center">
                    ...
                    <span>Weight</span>
                    <p className="mb-0">
                      {patient_data?.weight?.value} {patient_data?.weight?.unit}
                    </p>
                  </li>
                  <li className="bmi_card flex-grow-1">
                    <div className="link_image d-flex justify-content-center align-items-center">
                ...
                <div className="footer_bmi">
                  {patientReportsPermissions?.canRead && !hideItems && (
        ...
        <div className="cmn_head mb-3 mt-4 position-relative">
          ...
          <div className="d-flex gap-2  position-absolute end-0 bg-white ps-3">
            {patientPlanPermissions?.canUpdate &&
              !hideItems &&
              canEditPatientPlan &&
              isPatientPlanEditableResponse?.isSuccess && (
                <button
                  className="cmn_btn filter_btn mt-3"
                  onClick={() =>
      ...
      {showAddPateintExercise && (
        <AddPateintExercise
          showAddPateintExercise={showAddPateintExercise}
          setshowAddPateintExercise={setshowAddPateintExercise}
          exercise_category={exercise_category}
          patientId={patientId}
          weekdays={weekdays}
          body_parts={body_parts}
